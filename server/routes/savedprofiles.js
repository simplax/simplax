...
router.post('/', (req, res, next) => {
  let { _owner, title, parallaxDataCode } = req.body
  SavedProfile.create({ _owner, title, parallaxDataCode })
    .then(savedprofile => {
      res.json({
        success: true,
        savedprofile
...
router.put('/:id', (req, res, next) => {
  let { _owner, title, parallaxDataCode } = req.body
  SavedProfile.findByIdAndUpdate(req.params.id, { _owner, title, parallaxDataCode })
    .then(savedprofile => {
      res.json({
        savedprofile
      })
